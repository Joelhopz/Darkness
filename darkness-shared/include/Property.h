...
public:
        ...
        , m_name{ name }
    {
        ...
        createVariantDrawer = [&](DrawerCallbacks callback)->std::shared_ptr<Visual>
        {
    ...
    template <class T>
    void value(typename const T& _value)
    {
        m_impl = std::make_shared<VariantImpl<typename std::decay<T>::type>>(_value);
    }
    ...
    std::function<std::shared_ptr<Visual>(DrawerCallbacks)> createVariantDrawer;
