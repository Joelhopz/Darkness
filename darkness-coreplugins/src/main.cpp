...
DLLEXPORT void CALLCONV initializeTypes(void* typeRegistry)
{
    ...
    registry->registerProperty<engine::Transform>(std::make_shared<Property>("position", engine::Vector3f{ 0.0f, 0.0f, 0.0f }));
    registry->registerProperty<engine::Transform>(std::make_shared<Property>("rotation", engine::Matrix4f::identity() ));
    registry->registerProperty<engine::Transform>(std::make_shared<Property>("scale", engine::Vector3f{ 1.0f, 1.0f, 1.0f }));
    registry->registerProperty<engine::Transform>(std::make_shared<Property>("name", std::string("Transform")));
}
