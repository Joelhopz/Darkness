...
TEST(TestRefCounted, RefCountedPerformance)
{
	struct tempstruct
	{
		Float4x4 a;
	...
	// ####################################################################
	std::vector<std::shared_ptr<tempstruct>> shared;
	for (int i = 0; i < 1000000; ++i)
	{
		shared.emplace_back(std::make_shared<tempstruct>(tempstruct{ Float4x4::zero(), Float4x4::zero(), Float4x4::zero() }));
		//shared.emplace_back(new tempstruct({ Float4x4::zero(), Float4x4::zero(), Float4x4::zero() }));
	}

	std::vector<std::shared_ptr<tempstruct>> shared_dst;
	...
	// ####################################################################
	std::vector<tools::RefCounted<tempstruct>> eshared;
	...
	std::vector<tools::RefCounted<tempstruct>> eshared_dst;
	...
	std::vector<tempstruct> inst_dst;
