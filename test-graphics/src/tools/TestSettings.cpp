...
TEST(TestSettings, Settings_JsonBackend)
{
    ...
    // save some settings
    {
        ...
        settings.set("testString", std::string("test string"));
    ...
    // load those settings
    {
        ...
        EXPECT_EQ(settings.get<std::string>("testString"), std::string("test string"));
        EXPECT_EQ(settings.get<float>("testFloat"), 1.234f);

        auto testVector = settings.get<std::vector<int>>("testVector");
...
{
    {
        ...
        settings.set("group2 value1", std::string("123"));
        settings.endGroup();
        ...
        settings.beginGroup("deep group level 0");
            settings.beginGroup("deep group level 1");
                settings.beginGroup("deep group level 2");
                    ...
                    settings.set("some value", std::string("abc"));
                    settings.endGroup();
                settings.endGroup();
            settings.endGroup();
        settings.endGroup();
...

    {
        ...
        settings.beginGroup("testGroup2");
        EXPECT_EQ(settings.get<std::string>("group2 value1"), std::string("123"));
        settings.endGroup();
        ...
        settings.beginGroup("deep group level 0");
            settings.beginGroup("deep group level 1");
                settings.beginGroup("deep group level 2");
                    ...
                    EXPECT_EQ(settings.get<std::string>("some value"), std::string("abc"));
                    settings.endGroup();
                settings.endGroup();
            settings.endGroup();
        settings.endGroup();
