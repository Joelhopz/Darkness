...
template<typename T>
std::string debugRange(T& range, int index)
{
    return "[alignment: " + std::to_string(indexToAlign(index)) + ", count: " + std::to_string(range.size()) + "]";
};

TEST(TestMemoryAllocator, DISABLED_MemoryAllocator_PerformanceTest)
{
    std::vector<uint8_t> heap(3ull * 1024ull * 1024ull * 1024ull);
    ...
    Measure measure;
    {
        measure.here(std::to_string(TestCount * 15)+" allocations");
        for (int i = 1; i < 16; ++i)
        {
            auto alignment = 1 << i;
    ...
    {
        measure.here(std::to_string(allocations.size())+" frees");
        std::mt19937 gen(456); //Standard mersenne_twister_engine seeded with rd()
