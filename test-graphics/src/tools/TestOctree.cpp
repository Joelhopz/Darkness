...
TEST(TestOctree, ClosestPayload)
{
    ...
    auto& flatScene = scene.flatten(false);

    engine::ImguiRenderer imguiRenderer(env.device(), env.shaderStorage());
    ...
    do
    {
        ...
        auto getBruteClosest = [&](int a)->int
        {
            ...
            auto cmd = env.device().createCommandList();
            ...
            for (int i = 0; i < points.size(); ++i)
            {
                ...
                cb = b;
#endif
                auto distA = (points[i] - points[a]).magnitude();
                auto distB = (points[i] - points[b]).magnitude();
            ...
            std::vector<BoundingSphere> other_spheres;
            for (int i = 0; i < points.size(); ++i)
            {
                if(i != cc && i != ca && i != cb)
            ...
            imguiRenderer.render(env.device(), flatScene);
