...
namespace engine
{
    ...
    void Matrix4fDrawer::setParent(void* parent)
    {
        if (parent)
        {
            ...
            m_widget = std::make_shared<Matrix4fWidget>(m_value, m_matrix, m_parent);
    ...
    {
        return std::make_shared<Matrix4fDrawer>(value, true);
    }

    template <>
    std::shared_ptr<Drawer> createDrawer<Quaternionf>(Property& value)
    {
        return std::make_shared<Matrix4fDrawer>(value, false);
    }
}
