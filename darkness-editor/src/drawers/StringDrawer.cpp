...
namespace engine
{
		...
		if (dropEvent->mimeData()->hasUrls())
		{
			for each (QUrl url in dropEvent->mimeData()->urls())
			{
				setText(assetFilePathUnderProcessed(
					QString::fromStdString(m_contentPath), 
					QString::fromStdString(m_processedContentPath),
					url.toLocalFile()));
				emit onChanged();
				break;
			}
	...
	void DroppableLabel::setPaths(const std::string& contentPath, const std::string& processedContentPath)
	{
    ...
    StringWidget::StringWidget(
        ...
        , m_xLine{ std::make_unique<DroppableLabel>(this) }
        ...
        , m_valueValid{ true }
    {
        ...
        m_xLine->setText(value.value<std::string>().data());
        ...
        value.registerForChangeNotification(this, [this]() { if (m_canReceiveUpdate) m_xLine->setText(m_value.value<std::string>().data()); });
    ...
    void StringWidget::updateProperty()
    {
        ...
        m_value.value<std::string>(m_xLine->text().toStdString());
        m_canReceiveUpdate = true;
    }
	...
	void StringWidget::setPaths(const std::string& contentPath, const std::string& processedContentPath)
	{
		m_contentPath = contentPath;
		m_processedContentPath = processedContentPath;
    ...
    void StringDrawer::setParent(void* parent)
    {
        if (parent)
        {
            ...
            m_widget = std::make_shared<StringWidget>(m_value, m_parent);
	...
	void StringDrawer::setPaths(const std::string& contentPath, const std::string& processedContentPath)
	{
		m_contentPath = contentPath;
		m_processedContentPath = processedContentPath;
		if (m_widget)
    ...
    {
        return std::make_shared<StringDrawer>(value);
    }
}
