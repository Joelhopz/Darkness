...
namespace engine
{
    ...
    ButtonToggleWidget::ButtonToggleWidget(
        ...
        , m_layout{ std::make_unique<QHBoxLayout>() }
        , m_nameLabel{ std::make_unique<QLabel>(this) }
        , m_button{ std::make_unique<QCheckBox>(this) }
        , m_canReceiveUpdate{ true }
    {
        setLayout(m_layout.get());
        m_layout->setSpacing(0);
    ...
    void ButtonPushDrawer::setParent(void* parent)
    {
        if (parent)
        {
            ...
            m_widget = std::make_shared<ButtonPushWidget>(m_value, m_parent);
    ...
    void ButtonToggleDrawer::setParent(void* parent)
    {
        if (parent)
        {
            ...
            m_widget = std::make_shared<ButtonToggleWidget>(m_value, m_parent);
            m_widget->setParent(m_parent);
            m_widget->resize(m_parent->width(), 15);
            m_widget->setMaximumHeight(15);
            m_widget->setMinimumWidth(15);
    ...
    template <>
    std::shared_ptr<Drawer> createDrawer<engine::ButtonPush>(Property& value)
    {
        return std::make_shared<ButtonPushDrawer>(value);
    }

    template <>
    std::shared_ptr<Drawer> createDrawer<engine::ButtonToggle>(Property& value)
    {
        return std::make_shared<ButtonToggleDrawer>(value);
    }
}
