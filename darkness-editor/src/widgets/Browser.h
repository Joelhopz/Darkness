...
public:
    FileViewToolbar(Settings& settings, QWidget *parent = Q_NULLPTR, Qt::WindowFlags f = Qt::WindowFlags())
        : QWidget(parent, f)
        , m_settings{ settings }
        , m_spacer{ std::make_unique<QWidget>() }
        , m_layout{ std::make_unique<QHBoxLayout>() }
        , m_slider{ std::make_unique<QSlider>() }
        , m_spacerRight{ std::make_unique<QWidget>() }
    {
...
private:
    Settings& m_settings;
    std::unique_ptr<QWidget> m_spacer;
    std::unique_ptr<QHBoxLayout> m_layout;
    std::unique_ptr<QSlider> m_slider;
    std::unique_ptr<QWidget> m_spacerRight;
};
...
private:
    ...
    std::unique_ptr<DarknessIconProvider> m_iconProvider;
    std::unique_ptr<QFileSystemModel> m_fileSystemDirModel;
    std::unique_ptr<QFileSystemModel> m_fileSystemFileModel;

    std::unique_ptr<QSplitter> m_mainWidget;
        std::unique_ptr<DarknessTreeView> m_directoryView;
        std::unique_ptr<QSplitter> m_fileViewWidget;
            std::unique_ptr<DarknessListView> m_fileView;
            std::unique_ptr<FileViewToolbar> m_toolbarFileView;
