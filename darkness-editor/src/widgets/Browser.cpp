...
Browser::Browser(
    ...
    , m_iconProvider{ std::make_unique<DarknessIconProvider>() }
    , m_fileSystemDirModel{ std::make_unique<QFileSystemModel>() }
    , m_fileSystemFileModel{ std::make_unique<QFileSystemModel>() }
    , m_mainWidget{ std::make_unique<QSplitter>(this) }
    , m_directoryView{ std::make_unique<DarknessTreeView>(m_mainWidget.get()) }
    , m_fileViewWidget{ std::make_unique<QSplitter>(m_mainWidget.get()) }
    , m_fileView{ std::make_unique<DarknessListView>(m_fileViewWidget.get()) }
    , m_toolbarFileView{ std::make_unique<FileViewToolbar>(settings) }
{
    ...
    else if (m_directoryView->hasFocus() && keyEvent->key() == Qt::Key::Key_Delete)
    {
        ...
        if (confirmation == QMessageBox::StandardButton::Yes)
        {
            for (auto folder : indexes)
            {
                ...
                {
                    engine::Directory dir(folderToRemove.toStdString());
                    dir.remove(true);
                }
                ...
                {
                    engine::Directory dir(filePathUnderProcessed.toStdString());
                    dir.remove(true);
                }
