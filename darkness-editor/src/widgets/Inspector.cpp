...
void drawTextGlobal(void* propertyWidget, engine::Rect rect, const std::string& str)
{
...
PropertyWidget::PropertyWidget(
    ...
    , m_lineEdit{ std::make_unique<QLineEdit>(this) }
    , m_dragable{ std::make_unique<engine::DragableLabel>(*m_lineEdit.get(), this) }
{
...
void PropertyWidget::drawText(engine::Rect rect, const std::string& str)
{
    int test = 1;
}
...
ComponentWidget::ComponentWidget(
    ...
    , m_componentLabel{ std::make_unique<QLabel>() }
...
ComponentWidget::ComponentWidget(
    ...
    , m_processedPath{ processedPath }
{
    m_componentInstance = std::make_shared<engine::TypeInstance>(component);
    setLayout(m_layout.get());
    m_layout->setSpacing(4);
    m_layout->setMargin(0);
    m_layout->setContentsMargins(QMargins(4, 4, 4, 4));
...
void ComponentWidget::populate()
{
    for (auto& prop : m_component->propertyNames())
    {
        ...
        drawer->setPaths(m_contentPath.toStdString(), m_processedPath.toStdString());
...
Inspector::Inspector(
    ...
    , m_mainWidget{ std::make_unique<QWidget>(this) }
    , m_layout{ std::make_unique<QVBoxLayout>() }
{
...
void Inspector::nodeSelected(std::shared_ptr<engine::SceneNode> node)
{
    m_selectedNode = node;
    m_components.clear();
    //int hackHeight = 0;
    ...
    {
        m_components.append(std::make_shared<ComponentWidget>(node->component(i), m_contentPath, m_processedAssetsPathAbsolute, this));
    ...
    {
        m_components.append(std::make_shared<ComponentWidget>(component, this));
        m_layout->addWidget(m_components.last().get());
    }*/
}

std::shared_ptr<engine::EngineComponent> fileComponentFactory(
	...
	const QString& processedPath)
{
	...
	if (engine::isImageFormat(filepath.toStdString()))
	{
		return std::make_shared<engine::ImagePropertiesComponent>(
			assetFilePathUnderProcessed(contentPath, processedPath, filepath).toStdString());
	}
	else if (engine::isModelFormat(filepath.toStdString()))
	{
		return std::make_shared<engine::ModelPropertiesComponent>(
			assetFilePathUnderProcessed(contentPath, processedPath, filepath).toStdString());
	}
    else if (engine::isPrefabFormat(filepath.toStdString()))
    {
        // TODO ?
    }
...
void Inspector::fileSelected(const QString& filePath)
{
    ...
    if ((engine::isImageFormat(filePath.toStdString())) ||
        (engine::isModelFormat(filePath.toStdString())))
    {
        m_components.append(std::make_shared<ComponentWidget>(
...
void Inspector::ShowContextMenu(const QPoint& point)
{
    ...
    if (selectedItem)
    {
        ...
        {
            m_selectedNode->addComponent(std::make_shared<engine::RigidBodyComponent>());
        }
