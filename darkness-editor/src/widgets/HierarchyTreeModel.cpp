...
void HierarchyTreeModel::addNode(std::shared_ptr<engine::SceneNode> node)
{
    beginResetModel();
...
bool HierarchyTreeModel::dropMimeData(const QMimeData * data, Qt::DropAction action, int row, int column, const QModelIndex & parent)
{
    ...
    std::shared_ptr<engine::SceneNode> parentNode;
    if(parent.isValid())
    ...
    if (data->hasUrls())
    {
        ...
        for each (QUrl url in data->urls())
        {
            if (engine::isModelFormat(QFileInfo(url.toLocalFile()).fileName().toStdString()))
            {
                std::shared_ptr<engine::SceneNode> node = std::make_shared<engine::SceneNode>();
                node->addComponent(std::make_shared<Transform>());

                node->addComponent(std::make_shared<MeshRendererComponent>(
            ...
            else if (engine::isPrefabFormat(QFileInfo(url.toLocalFile()).fileName().toStdString()))
            {
                std::string scenePath = assetFilePathUnderContent(m_contentPath, m_processedPath, url.toLocalFile()).toStdString();
                engine::Scene scene;
...
std::shared_ptr<engine::SceneNode> HierarchyTreeModel::node(const QModelIndex& index)
{
