...
MainWindow::MainWindow(const CustomSizeHintMap &customSizeHints, const QString& projectPath,
    ...
    , m_assetImporter{ std::make_unique<AssetImporter>(m_settings.contentPathAbsolute(), m_settings.processedAssetsPathAbsolute()) }
    , m_engineWindow{ std::make_unique<EngineWindow>(this, Qt::WindowFlags(), m_settings.shaderPathAbsolute()) }
    , m_browser{ std::make_unique<Browser>(m_settings, this, this) }
    , m_hierarchy{ std::make_unique<Hierarchy>(m_settings, m_engineWindow->engine(), this, this) }
    , m_inspector{ std::make_unique<Inspector>(m_settings, this, this) }
    , m_decodePopup{ std::make_unique<AssetDecodePopup>(this) }
{
    ...
    QObject::connect(
        m_hierarchy.get(), SIGNAL(nodeSelected(std::shared_ptr<engine::SceneNode>)),
    ...
    {
        if (engine::fileExists(lastLoadedScenePath.toStdString()))
        {
            m_hierarchy->beginModelReset();
            m_engineWindow->engine().scene().loadFrom(lastLoadedScenePath.toStdString());
...
void MainWindow::saveScene()
{
    ...
    m_engineWindow->engine().scene().saveTo(fileName.toStdString());
}
