...
{
#ifdef _WIN32
    void PluginManager::addFolder(const std::string& folder)
    {
#if 1
        Directory directory(folder);
        for (auto file : directory.files())
        {
            std::string libPath = folder + file;

            size_t marker = libPath.find_last_of('.') + 1;
            std::string fileExtension = libPath.substr(marker, libPath.length() - marker);
        ...
        , m_deserializeType{ nullptr }*/
    {
        ...
        {
            m_pluginTypes.emplace_back(std::make_shared<PluginType>( m_apiPointers, i ));
        }
    }
#else
    void PluginManager::addFolder(std::string folder)
    {
    }
