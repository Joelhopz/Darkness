...
namespace platform
{
    Socket::Socket(uintptr_t socket)
        : m_socket{ socket }
        , m_sockAddr{ std::make_shared<sockaddr_in>() }
        , m_nonBlocking{ false }
    {}

    std::string getReadErrorString(int errorCode)
    {
        switch (errorCode)
        {
        case WSANOTINITIALISED: { return std::string("A successful WSAStartup call must occur before using this function."); }
        case WSAENETDOWN: { return std::string("The network subsystem has failed."); }
        case WSAEFAULT: { return std::string("The buf parameter is not completely contained in a valid part of the user address space."); }
        case WSAENOTCONN: { return std::string("The socket is not connected."); }
        case WSAEINTR: { return std::string("The(blocking) call was canceled through WSACancelBlockingCall."); }
        case WSAEINPROGRESS: { return std::string("A blocking Windows Sockets 1.1 call is in progress, or the service provider is still processing a callback function."); }
        case WSAENETRESET: { return std::string("For a connection - oriented socket, this error indicates that the connection has been broken due to keep - alive activity that detected a failure while the operation was in progress.For a datagram socket, this error indicates that the time to live has expired."); }
        case WSAENOTSOCK: { return std::string("The descriptor is not a socket."); }
        case WSAEOPNOTSUPP: { return std::string("MSG_OOB was specified, but the socket is not stream - style such as type SOCK_STREAM, OOB data is not supported in the communication domain associated with this socket, or the socket is unidirectional and supports only send operations."); }
        case WSAESHUTDOWN: { return std::string("The socket has been shut down; it is not possible to receive on a socket after shutdown has been invoked with how set to SD_RECEIVE or SD_BOTH."); }
        case WSAEWOULDBLOCK: { return std::string("The socket is marked as nonblocking and the receive operation would block."); }
        case WSAEMSGSIZE: { return std::string("The message was too large to fit into the specified buffer and was truncated."); }
        case WSAEINVAL: { return std::string("The socket has not been bound with bind, or an unknown flag was specified, or MSG_OOB was specified for a socket with SO_OOBINLINE enabled or (for byte stream sockets only) len was zero or negative."); }
        case WSAECONNABORTED: { return std::string("The virtual circuit was terminated due to a time - out or other failure.The application should close the socket as it is no longer usable."); }
        case WSAETIMEDOUT: { return std::string("The connection has been dropped because of a network failure or because the peer system failed to respond."); }
        case WSAECONNRESET: { return std::string("The virtual circuit was reset by the remote side executing a hard or abortive close.The application should close the socket as it is no longer usable.On a UDP - datagram socket, this error would indicate that a previous send operation resulted in an ICMP \"Port Unreachable\" message."); }
        default: return std::string("socket send unknown error");
        }
    }
    ...
    size_t Socket::write(const char* buffer, size_t lengthBytes)
    {
        ...
        std::vector<char> data(lengthBytes);
    ...
    std::string getSendErrorString(int errorCode)
    {
        switch (errorCode)
        {
        case WSANOTINITIALISED: { return std::string("A successful WSAStartup call must occur before using this function."); break; }
        case WSAENETDOWN: { return std::string("The network subsystem has failed."); break; }
        case WSAEACCES: { return std::string("The requested address is a broadcast address, but the appropriate flag was not set. Call setsockopt with the SO_BROADCAST socket option to enable use of the broadcast address."); break; }
        case WSAEINTR: { return std::string("A blocking Windows Sockets 1.1 call was canceled through WSACancelBlockingCall."); break; }
        case WSAEINPROGRESS: { return std::string("A blocking Windows Sockets 1.1 call is in progress, or the service provider is still processing a callback function."); break; }
        case WSAEFAULT: { return std::string("The buf parameter is not completely contained in a valid part of the user address space."); break; }
        case WSAENETRESET: { return std::string("The connection has been broken due to the keep-alive activity detecting a failure while the operation was in progress."); break; }
        case WSAENOBUFS: { return std::string("No buffer space is available."); break; }
        case WSAENOTCONN: { return std::string("The socket is not connected."); break; }
        case WSAENOTSOCK: { return std::string("The descriptor is not a socket."); break; }
        case WSAEOPNOTSUPP: { return std::string("MSG_OOB was specified, but the socket is not stream-style such as type SOCK_STREAM, OOB data is not supported in the communication domain associated with this socket, or the socket is unidirectional and supports only receive operations."); break; }
        case WSAESHUTDOWN: { return std::string("The socket has been shut down; it is not possible to send on a socket after shutdown has been invoked with how set to SD_SEND or SD_BOTH."); break; }
        case WSAEWOULDBLOCK: { return std::string("The socket is marked as nonblocking and the requested operation would block."); break; }
        case WSAEMSGSIZE: { return std::string("The socket is message oriented, and the message is larger than the maximum supported by the underlying transport."); break; }

        case WSAEHOSTUNREACH: { return std::string("The remote host cannot be reached from this host at this time."); break; }
        case WSAEINVAL: { return std::string("The socket has not been bound with bind, or an unknown flag was specified, or MSG_OOB was specified for a socket with SO_OOBINLINE enabled."); break; }
        case WSAECONNABORTED: { return std::string("The virtual circuit was terminated due to a time-out or other failure. The application should close the socket as it is no longer usable."); break; }
        ...
        case WSAETIMEDOUT: { return std::string("The connection has been dropped, because of a network failure or because the system on the other end went down without notice."); break; }
        }
    ...
    std::string Socket::localHostname()
    {
    ...
    std::string Socket::localIp()
    {
        ...
        for (int i = 0; phe->h_addr_list[i] != 0; ++i) {
            ...
            auto ipstr = std::string(inet_ntoa(addr));
            //LOG_INFO("Local IP address: %s", ipstr.c_str());

            if (ipstr.find("127.0.0.1") == std::string::npos)
            {
                result = ipstr;
            }
        }
    ...
    std::shared_ptr<Socket> Socket::createSocket(int port, const std::string& ip, SocketType type)
    {
        ...
        auto res = std::shared_ptr<Socket>(
    ...
    {
        std::vector<char> m_temporaryData(dataAvailableInSocket());
