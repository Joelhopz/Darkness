...
namespace platform
{
        ...
        , m_onData{ onData }
    {
        ...
        m_parser = std::make_shared<PrefixLengthMessageParser>(m_onData);
        ...
        m_closingDownMessage = std::unique_ptr<char, std::function<void(char*)>>(new char(), [](char* ptr)
        {
    ...
    void SocketClient::sendMessage(std::shared_ptr<platform::Socket> socket, const NetworkMessage& message)
    {
        m_parser->sendMessage(socket, message);
    }

    std::shared_ptr<Socket>& SocketClient::socket()
    {
