...
namespace platform
{
    ...
    void PrefixLengthMessageParser::setSocket(std::shared_ptr<platform::Socket> socket)
    {
        m_socket = socket;
    }

    void PrefixLengthMessageParser::sendMessage(std::shared_ptr<platform::Socket> socket, const NetworkMessage& message)
    {
        ASSERT(message.bytes > 0, "Weird message");
        std::vector<char> buffer(message.bytes + sizeof(uint32_t));
    ...
    void PrefixLengthMessageParser::onData(std::shared_ptr<platform::Socket> socket, const char* buffer, size_t bytes)
    {
