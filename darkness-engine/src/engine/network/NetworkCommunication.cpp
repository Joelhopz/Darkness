...
namespace engine
{
    ...
    {
        {
            lock_guard<mutex> lock(m_commMutex);
            m_alive = false;
        }
        m_commThread.join();
    }
    ...
    {
        m_context = make_unique<context_t>(context_t{ 1 });
        m_router = make_unique<socket_t>(socket_t{ *m_context, socket_type::router });
        ...
        while (alive)
        {
            ...
            if (haveTask)
            {
                switch (task.type)
                {
                    case CommTaskType::ClientEntry:
                    {
                        ...
                        m_router->connect(addressFromIp(task.beacon.ip()));

                        LOG("HOST Connecting to TASK DISTRIBUTOR: %s, ID: %s", 
                            addressFromIp(task.beacon.ip()).c_str(),
                    ...
                    case CommTaskType::ClientLeave:
                    {
                        ...
                        m_router->disconnect(addressFromIp(task.beacon.ip()));
                        LOG("HOST Disconnecting from TASK DISTRIBUTOR: %s", addressFromIp(task.beacon.ip()).c_str());
                        break;
                    }
                    case CommTaskType::Send:
                    {
                        task.msg.prepend("");
                        task.msg.prepend(task.beacon.id());
...

            {
                ...
                for (auto&& client : m_clients)
                {
                    if (client.cores == 0)
                    {
                        ...
                        if (durationMs > 1000)
                        {
                            auto msg = msgTo(client.beacon.id(), HostProcessorMessageType::CoreRequest);
                            msg.send(*m_router);
                            client.lastSent = now;
                        }
                    }
            ...
            if (m_polls[0].revents & ZMQ_POLLIN)
            {
                ...
                if (type.type() == HostProcessorMessageType::CoreResponse)
                {
                    ...
                    answer.ParseFromArray(msg.parts()[2].data(), msg.parts()[2].size());
                    ...
                    bool foundHel = false;
                    {
                        ...
                        for (auto&& client : m_clients)
                        {
                            if (client.beacon.id() == id && client.cores == 0)
                            {
                ...
                if (type.type() == HostProcessorMessageType::TaskStarted)
                {
                    ...
                    start.ParseFromArray(msg.parts()[2].data(), msg.parts()[2].size());
                    if (m_started)
                        m_started(start.taskid());
                }
                if (type.type() == HostProcessorMessageType::TaskFinished)
                {
                    HostTaskFinished finished;
                    finished.ParseFromArray(msg.parts()[2].data(), msg.parts()[2].size());
                    if (m_finished)
                        m_finished(finished.taskid());
                }
                if (type.type() == HostProcessorMessageType::TaskProgress)
                {
                    HostTaskProgress progress;
                    progress.ParseFromArray(msg.parts()[2].data(), msg.parts()[2].size());
                    if (m_progress)
                        m_progress(progress.taskid(), progress.progress());
                }
                if (type.type() == HostProcessorMessageType::TaskProgressMessage)
                {
                    ...
                    if (m_progressMessage)
                        m_progressMessage(progress.taskid(), progress.progress(), progress.message());
                }
                if (type.type() == HostProcessorMessageType::TaskImageResponse)
                {
                    HostTaskImageResponse resp;
                    resp.ParseFromArray(msg.parts()[2].data(), msg.parts()[2].size());
                    if (m_imageResult)
                        m_imageResult(resp);
                }
    ...
    std::string NetworkCommunication::addressFromIp(const std::string& ip)
    {
        string address = "tcp://";
        address += ip;
        address += ":12134";
