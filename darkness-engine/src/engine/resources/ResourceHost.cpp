...
namespace engine
{
    ResourceHost::ResourceHost()
        ...
        , m_netComm {
            ...
            {
                lock_guard<mutex> lock(this->m_clientMutex);
            ...
            {
                lock_guard<mutex> lock(m_tasksListMutex);
            ...
            {
                lock_guard<mutex> lock(m_tasksListMutex);
                ASSERT(m_tasks.find(response.taskid()) != m_tasks.end(), "Got task results for task that does not exist");
                ProcessingTask& task = *m_tasks[response.taskid()];
            ...
            [this](HostTaskModelResponse response)
            {
                ...
                ofstream mesh;
                mesh.open(task.item.absoluteProcessedFilepath, ios::out | ios::binary);
                if (mesh.is_open())
                {
                ...
                ofstream prefab;
                prefab.open(prefabPath, ios::out | ios::binary);
                if (prefab.is_open())
                {
    ...
    }
    {
        ...
        m_delayedLocalResourceClientStart = move(unique_ptr<thread, function<void(thread*)>>(new thread(
            // task
            [this]()
            {
                this->delayedStart();
    ...
    void ResourceHost::startLocalResourceClient()
    {
        ...
        m_resourceClientProcess = make_shared<Process>(
            clientPath,
            args,
    ...
    void ResourceHost::stopLocalResourceClient()
    {
        ...
        BeaconHello beacon;
        beacon.set_ip(Socket::localIp());
        beacon.set_id(m_localResourceProcessId);
    ...
    void ResourceHost::taskHandler()
    {
        ...
        while (taskAlive || !taskQueueEmpty || m_taskWorkedOn)
        {
            ...
            this_thread::sleep_for(chrono::milliseconds(100));
            {
                ...
                {
                    m_workItem = move(m_taskQueue.front());
                    m_taskQueue.pop_front();
                    m_taskWorkedOn = true;

                    m_taskWorkerThread = move(unique_ptr<thread, function<void(thread*)>>(new thread(
                        // task
                        [this]()
                        {
    ...
    {
        lock_guard<mutex> lock(m_taskMutex);
        lock_guard<mutex> tasklock(m_tasksListMutex);
    ...
    void ResourceHost::taskWorkerHandler()
    {
        ...
        while (item.valid)
        {
            // process item
            {
                ...
                auto newTask = make_unique<ProcessingTask>();
    ...
    void ResourceHost::sendTask(BeaconHello& beacon, ProcessingTask* task)
    {
        ...
        if (engine::isImageFormat(task->item.absoluteSourceFilepath))
        {
            ...
            req.set_taskid(task->id);
            req.set_extension(pathExtractExtension(task->item.absoluteSourceFilepath));
        ...
        else if (engine::isModelFormat(task->item.absoluteSourceFilepath))
        {
            ...
            req.set_taskid(task->id);
            req.set_modeltargetpath(task->item.absoluteProcessedFilepath);
            req.set_assetname(plainFilename);
    ...
    BeaconHello ResourceHost::getAvailableClient(const std::string& taskId)
    {
