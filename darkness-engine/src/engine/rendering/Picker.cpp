...
namespace engine
{
    Picker::Picker(
        ...
        : m_pickPipeline{ device.createPipeline<shaders::Pick>(shaderStorage) }
        ...
        uint32_t mouseY)
    {
...

        {
            ...
            cmd.copyBuffer(m_pickUAV.buffer(), m_pickReadBack.buffer(), 1);
        }
    }
    ...
    {
        unsigned int res = 0;
        uint32_t* bufferPtr = reinterpret_cast<uint32_t*>(m_pickReadBack.buffer().map(device));
        res = *bufferPtr;
        m_pickReadBack.buffer().unmap(device);
        return res;
    }
}
