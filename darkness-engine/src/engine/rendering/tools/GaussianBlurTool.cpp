...
namespace engine
{
    GaussianBlurTool::GaussianBlurTool(Device& device, ShaderStorage& shaderStorage)
        : m_device{ device }
    ...
    int GaussianBlurTool::refreshInternalTextures(TextureSRV& src)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            container.m_blurTargetSRV[i] = m_device.createTextureSRV(container.m_blurTargetRTV[i].texture());
        }
