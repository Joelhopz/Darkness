...
namespace engine
{
    ModelResources::ModelResources(Device& device)
        ...
        , m_allocationState{ std::make_shared<ModelResourceAllocationState>() }
    {
    ...
    void ModelResources::updateSubmeshTransform(ModelAllocation& model, const Matrix4f& transform)
    {
        ...
        std::unique_ptr<TransformHistory> mat = std::make_unique<TransformHistory>();
    ...
    void ModelResources::updateSubmeshUV(ModelAllocation& model, uint32_t uv)
    {
#if 1
        std::unique_ptr<uint32_t> uvValue = std::make_unique<uint32_t>();
        *uvValue = uv;
        ...
        m_pendingUV.emplace_back(PendingUpload{
            tools::ByteRange(
                reinterpret_cast<uint8_t*>(uvValue.get()),
                reinterpret_cast<uint8_t*>(uvValue.get()) + sizeof(uint32_t)),
            range });
    ...
    {
#if 1
        std::unique_ptr<InstanceMaterial> mat = std::make_unique<InstanceMaterial>();
        *mat = material;
    ...
    {
#if 0
        std::unique_ptr<uint32_t> mat = std::make_unique<uint32_t>();
        *mat = objectId;
        ...
        std::unique_ptr<uint32_t> objmat = std::make_unique<uint32_t>();
    ...
    uint32_t ModelResources::addMaterial(const ResourceKey& key, image::ImageIf* image, bool srgb)
    {
        auto index = m_textures.push(key, m_device.createTextureSRV(key, TextureDescription()
    ...
    std::shared_ptr<ModelResources::ModelAllocation> ModelResources::addSubmesh(std::shared_ptr<engine::SubMeshInstance>& subMesh)
    {
        std::shared_ptr<ModelAllocation> resultPtr = std::shared_ptr<ModelAllocation>(new ModelAllocation(), [this](const ModelAllocation* ptr) { this->removeSubmesh(ptr); });
        ...
        std::shared_ptr<ModelAllocation> resultPtr = std::shared_ptr<ModelAllocation>(new ModelAllocation(), 
        ...
        std::unique_ptr<ClusterMaterial> material = std::make_unique<ClusterMaterial>();
        material->albedo = 0;
        ...
        // setup object id
        std::unique_ptr<uint32_t> materialIdToObjectId = std::make_unique<uint32_t>();
        m_pendingMaterialIdToObjectId.emplace_back(PendingUpload{
            tools::ByteRange(
        ...
        // setup object id
        std::unique_ptr<uint32_t> objectIdToMaterialId = std::make_unique<uint32_t>();
        m_pendingObjectIdToMaterialId.emplace_back(PendingUpload{
            tools::ByteRange(
        ...
        for (int i = 0; i < subMesh.clusterIndexCount.size(); ++i)
        {
            ...
            while (indexesProcessed < indexesToProcess)
            {
                ...
                std::unique_ptr<std::vector<uint32_t>> tempIndexes = std::make_unique<std::vector<uint32_t>>();
                ...
                auto bs = std::make_unique<BoundingSphere>(Vector4f{ center.x, center.y, center.z, radius.magnitude() });
                ...
                auto bb = std::make_unique<GpuBoundingBox>();
                ...
                std::unique_ptr<ClusterBinding> binding = std::make_unique<ClusterBinding>();
