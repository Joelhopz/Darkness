...
namespace engine
{
    ...
    void CollisionShapeComponent::refreshShape()
    {
        ...
        {
        case CollisionShape::Plane: { m_collisionShape = std::make_shared<btStaticPlaneShape>(btVector3{ 0.0f, 1.0f, 0.0f }, 0.0f); break; }
        case CollisionShape::Box: { m_collisionShape = std::make_shared<btBoxShape>(btVector3(btScalar(1.), btScalar(1.), btScalar(1.))); break; }
        case CollisionShape::Capsule: { m_collisionShape = std::make_shared<btCapsuleShape>(1.0f, 1.0f); break; }
        case CollisionShape::Sphere: { m_collisionShape = std::make_shared<btSphereShape>(1.0f); break; }
        case CollisionShape::Cylinder: { m_collisionShape = std::make_shared<btCylinderShape>(btVector3(0.5f, 0.5f, 0.5f)); break; }
        case CollisionShape::Cone: { m_collisionShape = std::make_shared<btConeShape>(1.0f, 1.0f); break; }

        case CollisionShape::BvhTriangleMesh: { /*m_collisionShape = std::make_shared<btBvhTriangleMeshShape>();*/ break; }
        case CollisionShape::ConvexHull: { /*m_collisionShape = std::make_shared<btConvexHullShape>();*/ break; }
        case CollisionShape::ConvexTriangleMesh: { /*m_collisionShape = std::make_shared<btConvexTriangleMeshShape>();*/ break; }
        }
    }

    std::shared_ptr<engine::EngineComponent> CollisionShapeComponent::clone() const
    {
        auto res = std::make_shared<engine::CollisionShapeComponent>();
