...
#define ISBITMASK( r,g,b,a ) ( ddpf.RBitMask == r && ddpf.GBitMask == g && ddpf.BBitMask == b && ddpf.ABitMask == a )
        ...
        bool Dds::readHeader(std::fstream& file)
        {
            ...
            if(hasDX10Header(m_header))
            {
                ...
                if (m_format == Format::Format_UNKNOWN)
                {
            ...
            return m_format != Format::Format_UNKNOWN;
        }
        ...
        {
            reserve();
            file.read(reinterpret_cast<char*>(m_data.data()), static_cast<streamsize>(m_data.size()));
        }
            ...
            uint32_t arraySlice) const
        {
            auto surfaceInfo = surfaceInformation(
                m_format,
                max<unsigned int>(m_header.width >> mipLevel, 1u),
                max<unsigned int>(m_header.height >> mipLevel, 1u));

            ImageSubresource res;
            res.width = max<unsigned int>(m_header.width >> mipLevel, 1u);
            res.height = max<unsigned int>(m_header.height >> mipLevel, 1u);
            res.slicePitch = slicePitch();
            res.sizeBytes = surfaceInfo.numBytes;
            res.pitch = surfaceInfo.rowBytes;

            uint32_t bytes = arraySlice * static_cast<uint32_t>(slicePitch());
            auto width = m_header.width;
            auto height = m_header.height;
        ...
        void Dds::createHeader(unsigned int width, unsigned int height, Format format)
        {
            ...
            m_header.flags = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT | DDSD_MIPMAPCOUNT | DDSD_PITCH;
                //| DDSD_LINEARSIZE |
                //| DDS_ALPHAPIXELS | 
                //| DDS_ALPHA |
                //| DDSCAPS_TEXTURE;
            m_header.pitchOrLinearSize = static_cast<unsigned int>(formatBytes(format, width, 1));
        ...
        void Dds::save(const char* data, size_t bytes)
        {
            ...
            std::fstream input;
            input.open(m_filename, ios::binary | ios::out);
            if (input.is_open())
            {
                ...
                input.write(data, static_cast<streamsize>(bytes));
                input.close();
            }
        }
        ...
        void Dds::save()
        {
            ...
            if (input.is_open())
            {
                ...
                input.write(reinterpret_cast<char*>(&m_header), sizeof(DdsHeader));
                input.write(reinterpret_cast<char*>(&m_extendedHeader), sizeof(DdsHeaderDXT10));
                input.write(reinterpret_cast<char*>(m_data.data()), static_cast<streamsize>(m_data.size()));
                input.close();
            }
        }
        ...
        /*void Dds::writePixels(std::fstream& file)
        {
            ...
            for(int y = (int)m_header.height - 4; y >= 0; y -= 4)
            {
                for(int x = 0; x < (int)m_header.width; x += 4)
                {
                    ...
                    std::vector<engine::Color4f>&& block = getBlock(x, y);
        ...
        /*std::vector<engine::Color4f> Dds::getBlock(int xp, int yp)
        {
            std::vector<engine::Color4f> colorBlock;
        ...
        void Dds::getMinMax(const std::vector<engine::Color4f>& block, engine::Color4f& minColor, engine::Color4f& maxColor)
        {
