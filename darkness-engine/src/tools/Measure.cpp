...
namespace tools
{
    namespace implementation
    {
        ...
        bool MeasureStorage::addMeasure(const std::string& key, uint32_t period, vector<Measurement> measurements)
        {
            vector<vector<Measurement>>& list = m_storage[key];
            list.emplace_back(measurements);
            return list.size() >= period;
        }
        void MeasureStorage::clearMeasure(const std::string& key)
        {
            m_storage.erase(key);
        }
        vector<double> MeasureStorage::average(const std::string& key)
        {
    ...
    void Measure::here(const std::string& msg)
    {
        if (m_active)
    ...
    Measure::~Measure()
    {
        ...
        if (m_active)
        {
            if (m_period != 1)
            {
                std::string key = m_measurements[0].msg ? m_measurements[0].msg : m_measurements[0].stdMsg;
