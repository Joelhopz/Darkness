...
namespace engine
{
    namespace implementation
    {
        SamplerImpl::SamplerImpl(
            ...
            : m_descriptorHandle{ DeviceImplGet::impl(device).heaps().sampler->getDescriptor() }
        {
            ...
            DeviceImplGet::impl(device).device()->CreateSampler(
                &samplerDesc,
                m_descriptorHandle.cpuHandle());
        }

        D3D12_CPU_DESCRIPTOR_HANDLE& SamplerImpl::native()
        {
            return m_descriptorHandle.cpuHandle();
        }

        const D3D12_CPU_DESCRIPTOR_HANDLE& SamplerImpl::native() const
        {
            return m_descriptorHandle.cpuHandle();
        }
    }
