...
namespace engine
{
    namespace implementation
    {
        DescriptorHeapImpl::DescriptorHeapImpl(
            ...
            , m_heap{ vulkanPtr<VkDescriptorPool>(DeviceImplGet::impl(device).device(), vkDestroyDescriptorPool) }
        {
            std::vector<VkDescriptorPoolSize> poolSize;
            poolSize.resize(type.size());
            for(size_t i = 0; i < type.size(); ++i)
            {
                poolSize[i].type = vulkanDescriptorType(type[i]);
                poolSize[i].descriptorCount = static_cast<uint32_t>(m_handlesAvailable);
            }
            ...
            auto result = vkCreateDescriptorPool(DeviceImplGet::impl(device).device(), &poolInfo, nullptr, m_heap.get());
            ASSERT(result == VK_SUCCESS);
        }
        
        // this is so wrong
        tools::impl_ptr<DescriptorHandleImpl> DescriptorHeapImpl::allocate(
            ...
            int count*/)
        {
            ...
            std::vector<VkDescriptorSetLayout> layouts{ RootSignatureImplGet::impl(signature).layout() };
            ...
            ASSERT(vkAllocateDescriptorSets(DeviceImplGet::impl(m_device).device(), &allocInfo, &descriptorSet) == VK_SUCCESS);
            ...
            const VkBufferView views[2] = { BufferSRVImplGet::impl(pipelineConfig.vertexShader()->buffer_srvs()[0])->native() ,BufferSRVImplGet::impl(pipelineConfig.vertexShader()->buffer_srvs()[1])->native() };
            ...
            for (auto&& res : pipelineConfig.vertexShader()->buffer_uavs())
            {
                /*rootSignature[currentIndex].binding(currentIndex);
                rootSignature[currentIndex].descriptorType(DescriptorType::UniformTexelBuffer);
                rootSignature[currentIndex].visibility(static_cast<ShaderVisibility>(ShaderVisibilityBits::Vertex));
                ++currentIndex;*/
            }
            /*for (auto&& res : pipelineConfig.pixelShader()->texture_srvs())
            {
                rootSignature[currentIndex].binding(currentIndex);
                rootSignature[currentIndex].descriptorType(DescriptorType::StorageImage);
            ...
            for (auto&& res : pipelineConfig.pixelShader()->texture_uavs())
            {
                rootSignature[currentIndex].binding(currentIndex);
                rootSignature[currentIndex].descriptorType(DescriptorType::StorageImage);
                rootSignature[currentIndex].visibility(static_cast<ShaderVisibility>(ShaderVisibilityBits::Pixel));
            ...
            for (auto&& res : pipelineConfig.pixelShader()->buffer_srvs())
            {
                rootSignature[currentIndex].binding(currentIndex);
                rootSignature[currentIndex].descriptorType(DescriptorType::UniformTexelBuffer);
                rootSignature[currentIndex].visibility(static_cast<ShaderVisibility>(ShaderVisibilityBits::Pixel));
            ...
            for (auto&& res : pipelineConfig.pixelShader()->buffer_uavs())
            {
                rootSignature[currentIndex].binding(currentIndex);
                rootSignature[currentIndex].descriptorType(DescriptorType::UniformTexelBuffer);
                rootSignature[currentIndex].visibility(static_cast<ShaderVisibility>(ShaderVisibilityBits::Pixel));
            ...
            vkUpdateDescriptorSets(DeviceImplGet::impl(m_device).device(), static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);

            return tools::make_impl<DescriptorHandleImpl>(descriptorSet);
        }

        VkDescriptorPool& DescriptorHeapImpl::native()
        {
            return *m_heap;
        }
