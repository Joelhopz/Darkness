...
namespace engine
{
    namespace serialization
    {
        ...
        std::string keyPrefix(K key, T id = TypeId::InvalidId)
        {
            return std::to_string((int)key) + ":" + std::to_string((int)id);
        }
    }
    ...
    Projection readJsonValue_projection(const std::string& value);

    enum class CollisionShape;
    CollisionShape readJsonValue_collisionShape(const std::string& value);
