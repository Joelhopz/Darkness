...
{
#ifdef DEBUG_OCTREE
    extern std::vector<engine::BoundingBox> usedBoundingBoxes;
    extern std::vector<engine::BoundingBox> searchBoundingBoxes;
    extern std::vector<engine::BoundingBox> initialSearchBoundingBoxes;
#endif
    ...
    public:
        ...
        // for debugging
        std::vector<BoundingBox> recursiveBoundingBoxes() const;
    private:
        ...
        std::vector<OctreeEntry<Payload>> allFromBoundingBox(const engine::BoundingBox& bb, bool& foundRoot, const engine::BoundingBox& originalBB);
        std::vector<OctreeEntry<Payload>> getAll() const;
        BoundingBox m_bb;
        std::vector<OctreeEntry<Payload>> m_entry;
    ...
    template<typename Payload>
    std::vector<BoundingBox> Octree<Payload>::recursiveBoundingBoxes() const
    {
    ...
    template<typename Payload>
    std::vector<OctreeEntry<Payload>> Octree<Payload>::allFromBoundingBox(const engine::BoundingBox& bb, bool& foundRoot, const engine::BoundingBox& originalBB)
    {
        ...
        if (m_type == OctreeNodeType::Root)
        {
            ...
            std::vector<OctreeEntry<Payload>> res;
            for (auto&& child : m_childs)
            {
        ...
        if (m_type != OctreeNodeType::Root)
        {
            ...
            {
                auto minsize = std::min(std::min(originalBB.width(), originalBB.height()), originalBB.depth()) / 2.0f;
    ...
    template<typename Payload>
    std::vector<OctreeEntry<Payload>> Octree<Payload>::getAll() const
    {
        ...
        if (m_childs.size() > 0)
        {
            for (int i = 0; i < m_childs.size(); ++i)
            {
                if (!measured && m_childs[i]->m_bb.contains(point))
                {
                    ...
                    if (found && !measured)
                    {
                        auto bbmax = max(max(m_bb.width(), m_bb.height()), m_bb.depth());
                        bool foundRoot = false;
    ...
    void Octree<Payload>::insert(const Vector3f& point, Payload payload)
    {
        ...
        std::vector<std::shared_ptr<Octree>>* childs = &m_childs;
        auto insertAsChild = [this, childs](const Vector3f& point, Payload payload)
        {
            ...
            if (!inserted)
            {
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    {
                        m_childs.emplace_back(make_shared<Octree>(this, bb, point, payload));
                    }
                }
            }
        };
            ...
            for (int i = 0; i < 8; ++i)
            {
                ...
                if (bb.contains(point))
                {
                    ...
                    else
                    {
                        ...
                        m_childs.emplace_back(make_shared<Octree>(this, childBoundingBox(m_leafPos), m_entry[0].point, m_entry[0].payload));
