...
namespace engine
{
    ParticleTest::ParticleTest(Device& device, ShaderStorage& shaderStorage)
        : m_particlePipeline{ device.createPipeline<shaders::Particle>(shaderStorage) }
        ...
        , m_currentFrame{ 0 }
    {
        ...
        std::vector<Vector3f> particlePositions;
    ...
    std::string ParticleTest::createFilename(std::string basename, std::string extension, int number, int digits)
    {
        stringstream ss;
        ss << setw(digits) << setfill('0') << number;
        return basename + ss.str() + extension;
    }

    void ParticleTest::createParticles(Device& device, const std::vector<Vector3f>& positions)
    {
    ...
    void ParticleTest::loadTextures(Device& device)
    {
        ...
        for (int i = 0; i < 40; ++i)
        {
            ...
            m_particlePipeline.ps.diffuse.push(
                tools::hash(fullDiffusePath),
                loadTexture(device, fullDiffusePath));

            auto fullNormalPath = dataPath + createFilename(normalBaseName, formatExtension, i, 5);
            m_particlePipeline.ps.normal.push(
                tools::hash(fullNormalPath),
                loadTexture(device, fullNormalPath));

            auto fullAlphaPath = dataPath + createFilename(alphaBaseName, formatExtension, i, 5);
            m_particlePipeline.ps.alpha.push(
                tools::hash(fullAlphaPath),
                loadTexture(device, fullAlphaPath));

            auto topPath = dataPath + createFilename(topBaseName, formatExtension, i, 5);
            m_particlePipeline.ps.top.push(
                tools::hash(topPath),
                loadTexture(device, topPath));

            auto leftPath = dataPath + createFilename(leftBaseName, formatExtension, i, 5);
            m_particlePipeline.ps.left.push(
                tools::hash(leftPath),
                loadTexture(device, leftPath));

            auto bottomPath = dataPath + createFilename(bottomBaseName, formatExtension, i, 5);
            m_particlePipeline.ps.bottom.push(
                tools::hash(bottomPath),
                loadTexture(device, bottomPath));

            auto rightPath = dataPath + createFilename(rightBaseName, formatExtension, i, 5);
            m_particlePipeline.ps.right.push(
                tools::hash(rightPath),
                loadTexture(device, rightPath));
        }
    }

    TextureSRV ParticleTest::loadTexture(Device& device, string filepath)
    {
        ...
        BufferSRV& shadowVP)
    {
        ...
        cmd.draw(m_particles.desc().elements * 6);
    }
