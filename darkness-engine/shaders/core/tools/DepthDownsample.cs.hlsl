...
void main(uint groupIndex : SV_GroupIndex, uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    if (dispatchThreadID.x < usize.x && dispatchThreadID.y < usize.y)
    {
        ...
        float tiledepth = min(min(min(srcData.x, srcData.y), srcData.z), srcData.w);
    ...
    if ((parity & 1) == 0)
    {
        float tiledepth = min(min(min(dTile[groupIndex], dTile[groupIndex + 1]), dTile[groupIndex + 8]), dTile[groupIndex + 9]);
        dTile[groupIndex] = tiledepth;
    ...
    if ((parity & 3) == 0)
    {
        float tiledepth = min(min(min(dTile[groupIndex], dTile[groupIndex + 2]), dTile[groupIndex + 16]), dTile[groupIndex + 18]);
        dTile[groupIndex] = tiledepth;
    ...
    if ((parity & 7) == 0)
    {
        float tiledepth = min(min(min(dTile[groupIndex], dTile[groupIndex + 4]), dTile[groupIndex + 32]), dTile[groupIndex + 36]);
