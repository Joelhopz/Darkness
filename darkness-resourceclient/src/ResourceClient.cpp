...
namespace resource_client_old
{
    ResourceClient::ResourceClient()
        : m_server{ std::make_shared<SocketServer>(2134, false, false,
            [](std::shared_ptr<Socket> socket) { /*LOG("Connection from: %s", socket->ip().c_str());*/ },
            [](std::shared_ptr<Socket> socket) { /*LOG("Client %s disconnected", socket->ip().c_str());*/ },
            [this](std::shared_ptr<Socket> socket, const NetworkMessage& message)
            {
                this->onMessage(socket, message);
            })}
        , m_serverToHost{ std::make_shared<SocketServer>(2135, false, false,
            [](std::shared_ptr<Socket> socket) { /*LOG("Connection from: %s", socket->ip().c_str());*/ },
            [](std::shared_ptr<Socket> socket) { /*LOG("Client %s disconnected", socket->ip().c_str());*/ },
            [this](std::shared_ptr<Socket> socket, const NetworkMessage& message)
            {
        ...
        , m_discoveryServer{ std::make_shared<ResourceDiscoveryServer>() }
        , m_alive{ true }
    ...
    void ResourceClient::onMessageFromHost(std::shared_ptr<platform::Socket> socket, const platform::NetworkMessage& message)
    {
        ...
        switch (msg)
        {
            case ResourceClientMessage::TestHelo:
            {
                ...
                std::vector<char> buffer(sizeof(ResourceClientMessage));
            ...
            case ResourceClientMessage::GetCores:
            {
                ...
                std::vector<char> buffer(sizeof(ResourceClientMessage) + sizeof(uint32_t));
            ...
            case ResourceClientMessage::ImageEncodingTask:
            {
                ...
                container.taskProcess = std::make_shared<engine::Process>(
                    taskPath,
                    ResourceTaskArgumentsStart + m_localIp + ResourceTaskArgumentsStop + std::to_string(m_magicNumber),
    ...
    void ResourceClient::onMessage(std::shared_ptr<platform::Socket> socket, const NetworkMessage& message)
    {
        ASSERT(message.bytes >= sizeof(ResourceClientMessage), "Malformed Resource client message");
        ResourceClientMessage msg;
        memcpy(&msg, message.buffer, sizeof(ResourceClientMessage));
        switch (msg)
        {
            ...
            case ResourceClientMessage::ImageEncodingDone:
            {
                ...
                std::vector<char> data(sizeof(ResourceClientMessage));
