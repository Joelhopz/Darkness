...
namespace resource_client
{
    ...
    std::vector<SplitTask> ImageSplitter::splitImageTask(Task& container)
    {
        std::vector<SplitTask> result;

        FIMEMORY* imageMemory = FreeImage_OpenMemory(reinterpret_cast<BYTE*>(container.data.data()), container.data.size());
    ...
    TaskResult ImageSplitter::joinImageTask(std::vector<SplitTaskResult>& results)
    {
        // sort results
