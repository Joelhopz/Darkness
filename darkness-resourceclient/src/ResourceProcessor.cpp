...
{
    ResourceProcessor::ResourceProcessor(const std::string& frontIdentity)
    ...
    void ResourceProcessor::processTasks()
    {
        ...
        std::shared_ptr<TaskContainer> split = nullptr;
        bool taskAlive = true;
        while (taskAlive)
        {
            {
    ...
    void ResourceProcessor::runClient()
    {
        ...
        m_client = std::make_unique<ResourceClient>(m_frontIdentity);
        bool alive = true;
        while (alive)
        {
            {
            ...
            {
                lock_guard<mutex> lock(m_workerMutex);
                m_tasksForSplitting.push(std::make_shared<TaskContainer>(m_client->getTask()));
